...
if __name__ == "__main__":
    ...
    args.data_root_dir='/media/data1/maleilei/dataset'
    model_name = {1:'ADD_GCN', 2:'ResNet101_GMP', 3:'ResNet101_GAP', 4:'ADD_GCN_FC'}
    dataset_name = {1:'COCO2014', 2:'VOC2012', 3:'VOC2007', 4:'VG_100K', 5:'CoCoDataset', 6:'nuswide', 7:'mirflickr25k'}
    backbone = {1:'ResNet101', 2:'ResNeXt50-swsl', 3:'ResNeXt50_32x4d', 4:'GLResNet101'}
    args.model_name = model_name[4]  # model name
    args.data = dataset_name[3]
    ...
    args.warmup_scheduler = True

    if args.optimizer == 'SGD':
        args.lr = 0.01 # voc is 0.01 and coco is 0.05
        args.lrp = 0.1
        # args.epoch_step = [30, 40] # gap in voc
        args.epoch_step = [10, 20] # gmp in voc
        ...
        args.epoch_step = [15, 25]
    
    elif args.optimizer == 'AdamW' and args.data in ('mirflickr25k'):
        args.lr = 5 * 1e-5
        args.lrp = 0.1
        args.weight_decay = 1e-4
        args.epoch_step = [15, 25]

    # work = 'work3_without_FrozenBN'
    work = 'work5'
    args.save_dir = './checkpoint/' + args.data + '/' +args.model_name+'/' + work

    args.evaluate = {1: False, 2: True}[2]
    ...
    if args.optimizer == 'SGD' and args.data in ('VOC2007') and args.model_name in ('ADD_GCN_FC'):
        args.epoch_step = [10, 20] # gmp in voc
        args.epoch_step = [11, 25] # ADD_GCN with SGD in VOC
        args.lr = 0.01 # voc is 0.01 and coco is 0.05
        args.lrp = 0.01
        # args.resume = './checkpoint/COCO2014/checkpoint_best.pth'
        args.resume = './checkpoint/CoCoDataset/ADD_GCN_FC/work2/checkpoint_best.pth'

    if args.optimizer == 'AdamW' and args.data in ('VOC2007') and args.model_name in ('ADD_GCN_FC'):
        args.epoch_step = [10, 20] # gmp in voc
        args.epoch_step = [11, 25] # ADD_GCN with SGD in VOC
        args.lr = 1e-5 # voc is 0.01 and coco is 0.05
        args.lrp = 0.01
        # args.resume = './checkpoint/COCO2014/checkpoint_best.pth'
        args.resume = './checkpoint/CoCoDataset/ADD_GCN_FC/work2/checkpoint_best.pth'

    # args.image_size = 576
    # args.resume = './checkpoint/VG_100K/ADD_GCN_FC/work1/Epoch-12.pth'
    args.image_size = 448
    args.resume = './checkpoint/VOC2007/ADD_GCN_FC/work4/checkpoint_best.pth'
    # args.resume = './checkpoint/VOC2007/ResNet101_GAP/work1/checkpoint_best.pth'
    # args.resume='./checkpoint/CoCoDataset/ADD_GCN_FC/work1/checkpoint_best.pth'
